...
function AnnoucementTable() {
  ...
  const annoucemenData = useAnnoucement(
    value?.startDate ? value : generalValue.annoucementDate
  );
  ...
  const columns = [t("_no"), t("_announcement"), t("_date")];
  const monthNames = monthName();

  const rowsByDate = annoucemenData?.data?.reduce((acc, row) => {
