...
function BroadcastTable() {
  ...
  const broadcastData = useBroadcastStatic(
    value?.startDate ? value : generalValue.broadcastDate
  );
  ...
  const columns = [t("_no"), t("_broadcast"), t("_date")];
  const monthNames = monthName();

  const rowsByDate =
    broadcastData?.data &&
    broadcastData?.data?.reduce((acc, row) => {
