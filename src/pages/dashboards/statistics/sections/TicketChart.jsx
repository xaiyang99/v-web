...
import TicketTable from "../tableStatistic/TicketTable";
...
function TicketChart({ theme }) {
  ...
  const ticketData = useTicket(generalValue.ticketDate);
  ...
  React.useEffect(() => {
    if (selectedValue.refreshing) {
      ticketData.customeTicket();
    }
  }, [selectedValue.refreshing]);
  const dataDates = ticketData?.data?.map((item) =>
  ...
  const title = t("_support_ticket_menu");
        ...
        <CardHeader
          ...
          subheader={`${
            selectedValue.toggle !== "grid"
              ? subtitle
              : t("_total") + " " + ticketData?.data?.length
          }`}
          action={
            selectedValue.toggle !== "grid" && (
              <Sections
                selectedValue={generalValue.ticket}
                dispatch={handleSelectChange}
              />
            )
          }
        />
        {ticketData?.data?.length === undefined || selectedValue.refreshing ? (
          <Card
            sx={{
              height: isMobile ? "650px" : "600px",
              boxShadow: (theme) => theme.baseShadow.secondary,
            ...
            {selectedValue.toggle !== "grid" && (
              <Box sx={{ display: "block", marginLeft: "10px" }}>
                <Typography variant="h6" component="h6">
                  {formateViews(ticketData?.data?.length)}
                </Typography>
                <Typography
                  variant="p"
                  sx={{
...
export default withTheme(TicketChart);
